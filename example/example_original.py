...
def oncalls(token):
    uri = url + '/oncalls'
    print('oncalls')
    return print(requests.get(uri, headers={'X-Auth-Token': token}).json())


def action(token, cmds):
    uri = url + '/action'
    print('action', uri, cmds)
    return print(requests.post(uri, headers={'X-Auth-Token': token}, json={'commands': cmds}).json())
